...
- The demo dataset generated by `scripts/generate_demo.py` now ships with an
  **`RF`** column containing a low-volatility cash proxy. All demo configs set
  `data.risk_free_column: RF` and keep `allow_risk_free_fallback: false`, so the
  CLI and Streamlit examples behave identically to production environments.

When onboarding a new dataset, either provide a dedicated cash column or update
the config to opt into the heuristic fallback explicitly.
