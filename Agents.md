...
### 2025-07-25 UPDATE — multi-period sheets

The spreadsheet generated by the phase‑2 engine must mirror phase 1 output: every period gets its own tab with the same formatted summary table. The workbook also includes a `summary` tab showing the combined portfolio metrics in that identical layout. CSV and JSON exports contain all sheets collapsed into one file via a `sheet` column or key.

### 2025-07-30 UPDATE — multi-period output design

Phase 2 must replicate Phase 1\x27s summary table on every period sheet. All tabs (`period_1`, `period_2`, …) use the same header, column order and number formatting as the single-period export by calling `make_summary_formatter`. A final `summary` sheet aggregates portfolio-wide returns with that identical layout. CSV and JSON exports remain collapsed into one file with a `sheet` column or key.
