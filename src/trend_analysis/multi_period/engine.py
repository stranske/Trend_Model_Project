...
def run(
    ...
    *,
    ...
    for pt in periods:
        ...
        # Ensure intended holdings remain investable after turnover caps.
        # If turnover scaling zeroed out newly hired holdings, reinsert them
        # with the minimum weight floor so they remain in the portfolio.
        if min_funds > 0 and holdings:
            desired_holdings = [str(h) for h in holdings]
            final_w = final_w.reindex(desired_holdings, fill_value=0.0)
            missing = final_w.abs() <= NUMERICAL_TOLERANCE_HIGH
            if missing.any():
                final_w.loc[missing] = min_w_bound
                final_w = _apply_weight_bounds(final_w, min_w_bound, max_w_bound)
                final_w = _enforce_max_active_positions(
                    final_w, max_active_positions, protected=min_tenure_guard
                )
