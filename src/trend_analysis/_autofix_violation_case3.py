...
def compute_sum( a:int , b : int ) ->int:  # spacing & annotation style issues
    """Compute a sum (docstring intentionally single line, will be reformatted).
    Extra sentence to exceed typical wrap width so docformatter might reflow it significantly. Another sentence for good measure.
    """
    temp= a+ b
    return  temp
...
def ambiguous_types(x, y:List[int]):  # noqa: F405 (List from wildcard import)
    """Function with partially annotated parameters to trigger typing hygiene checks."""
