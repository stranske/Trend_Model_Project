...
def _format_nl_explanation(patch: ConfigPatch) -> str:
    ...
    if patch.needs_review and patch.operations:
        lines.append("Needs review: unknown or ambiguous keys detected.")
    rationales = [
...
def _confirm_risky_patch(patch: ConfigPatch, *, no_confirm: bool) -> None:
    ...
    if patch.risk_flags:
        flags = ", ".join(flag.value for flag in patch.risk_flags)
        reasons.append(f"risk flags: {flags}")
    if patch.needs_review and patch.operations:
        reasons.append("unknown or ambiguous keys")
    if not reasons or no_confirm:
        return
    reason_text = "; ".join(reasons)
    if not sys.stdin.isatty():
        raise TrendCLIError(
            "High-risk changes detected "
            f"({reason_text}). Re-run with --no-confirm to apply without prompting."
        )
    response = input(f"High-risk changes detected ({reason_text}). Continue? [y/N]: ")
