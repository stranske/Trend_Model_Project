...
- Update the default-branch protection rule so **both** `Gate / gate` and `Agents Guard / Enforce agents workflow protections` status checks are required simultaneously; no other contexts should be removed unless explicitly deprecated.
...
1. Default branch protection lists exactly `Gate / gate` and `Agents Guard / Enforce agents workflow protections` as required checks and keeps the `strict` flag enabled.
...
- [ ] Refactor or extend the enforcement helper so the desired contexts list includes both `Gate / gate` and `Agents Guard / Enforce agents workflow protections`, with reusable constants for downstream consumers.
