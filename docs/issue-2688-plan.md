...
- [x] Audit workflow permissions and token handling to confirm the job can operate with only `contents: read` and `issues: write`, gracefully skipping privileged checks when no PAT is provided.【F:.github/workflows/health-40-repo-selfcheck.yml†L13-L155】
