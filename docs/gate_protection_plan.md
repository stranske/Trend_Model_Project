...
- Enforce the default-branch protection rule so it always requires **Gate / gate**, **Enforce agents workflow protections**, and **Health 45 Agents Guard / Enforce agents workflow protections**.
...
- Default branch protection rule lists exactly the three contexts above and prevents merging until they succeed.
...
2. Update branch protection:
   - Add or confirm the required contexts: **Gate / gate**, **Enforce agents workflow protections**, and **Health 45 Agents Guard / Enforce agents workflow protections**.
...
- Added `tools/enforce_gate_branch_protection.py` to interrogate and update the default branch protection rule via the GitHub
  REST API so the **Gate / gate**, **Enforce agents workflow protections**, and **Health 45 Agents Guard / Enforce agents
  workflow protections** contexts remain required while "Require branches to be up to date" stays enabled. The helper accepts
...
- Scheduled automation (`.github/workflows/health-44-gate-branch-protection.yml`) executes the helper nightly and on-demand,
  ...
  step inspects the branch metadata API (which only requires repository read access) to confirm that all three contexts remain
  required.
...
Branch:     main
Current contexts: Gate / gate, Enforce agents workflow protections, Health 45 Agents Guard / Enforce agents workflow protections
Desired contexts: Gate / gate, Enforce agents workflow protections, Health 45 Agents Guard / Enforce agents workflow protections
