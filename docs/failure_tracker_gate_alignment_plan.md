...
- Consolidate tracker update logic into the existing `maint-46-post-ci.yml` pipeline while keeping backward-compatible hooks for the legacy tracker workflow until removal is validated.
...
- [x] Implement `ci-failure` label management in `maint-46-post-ci.yml` (apply on failure, remove when passing) with existing helper scripts or GitHub CLI usage.
...
- The Gate follower exposes `failure_tracker_skip` for PRs #10/#12 and the Maint 46 Post CI failure-tracker job requires the skip flag to be false before updating issues/labels.
- Maint 46 Post CI publishes a single `ci-failures-snapshot` artifact in both failure and success runs and is the exclusive writer of the maint-46-post-ci status comment, preventing duplicate bot posts.
