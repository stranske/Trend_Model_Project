...
- Consolidate tracker update logic into the existing `maint-30-post-ci.yml` pipeline while keeping backward-compatible hooks for the legacy tracker workflow until removal is validated.
...
2. On any qualifying Gate failure:
   - `maint-30-post-ci.yml` orchestrates the tracker update sequence and produces exactly one updated issue entry and one persistent PR comment.
...
- [x] Refactor tracker update steps into `maint-30-post-ci.yml`, preserving inputs, secrets, and retry/backoff behavior; leave a thin shell in the original workflow if needed for compatibility.
- [x] Wire `maint-30-post-ci.yml` to invoke the tracker update job conditionally based on Gate outcomes (failed, cancelled, neutral).
- [x] Implement `ci-failure` label management in `maint-30-post-ci.yml` (apply on failure, remove when passing) with existing helper scripts or GitHub CLI usage.
...
- Failure tracker issue/label management now lives in the new `failure-tracker` job within `maint-30-post-ci.yml`, preserving the signature hashing, cooldown, and escalation semantics from the legacy workflow.
...
- Maint Post CI publishes a single `ci-failures-snapshot` artifact in both failure and success runs and is the exclusive writer of the maint-30-post-ci status comment, preventing duplicate bot posts.
