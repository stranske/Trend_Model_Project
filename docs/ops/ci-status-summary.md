...
the `maint-post-ci.yml` follower completes. The workflow subscribes
...
* Additional artifacts can be surfaced by extending the artifact download steps
  in `maint-post-ci.yml` and updating the Markdown rendering helpers
...
Delete or rename `.github/workflows/maint-post-ci.yml` to disable the
