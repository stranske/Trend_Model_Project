...
- Label Agent PRs — [`agents-46-label-agent-prs.yml`](../../.github/workflows/agents-46-label-agent-prs.yml) · [jump](#wf-label-agent-prs)
- Merge Manager — [`maint-46-merge-manager.yml`](../../.github/workflows/maint-46-merge-manager.yml) · [jump](#wf-merge-manager)
...
- Cleanup Codex Bootstrap — [`maint-31-codex-bootstrap-cleanup.yml`](../../.github/workflows/maint-31-codex-bootstrap-cleanup.yml) · [jump](#wf-maint-31-codex-bootstrap-cleanup)
- Quarantine TTL — [`maint-34-quarantine-ttl.yml`](../../.github/workflows/maint-34-quarantine-ttl.yml) · [jump](#wf-quarantine-ttl)
- Verify Service Bot PAT — [`agents-48-verify-service-bot-pat.yml`](../../.github/workflows/agents-48-verify-service-bot-pat.yml) · [jump](#wf-verify-service-bot-pat)
- Copilot Readiness — [`agents-45-copilot-readiness.yml`](../../.github/workflows/agents-45-copilot-readiness.yml) · [jump](#wf-copilot-readiness)
- Verify Codex Bootstrap Matrix — [`agents-47-verify-codex-bootstrap-matrix.yml`](../../.github/workflows/agents-47-verify-codex-bootstrap-matrix.yml) · [jump](#wf-verify-codex-bootstrap-matrix)
...
- Release — [`maint-50-release.yml`](../../.github/workflows/maint-50-release.yml) · [jump](#wf-release)
...
4) [`agents-46-label-agent-prs.yml`](../../.github/workflows/agents-46-label-agent-prs.yml) — Apply agent labels to PRs (keeps downstream automation deterministic)
  ...
  - Links to: `maint-46-merge-manager.yml`, `maint-32-autofix.yml` (they predicate on these labels)

<a id="wf-merge-manager"></a>
4) [`maint-46-merge-manager.yml`](../../.github/workflows/maint-46-merge-manager.yml) — Unified merge automation
...
<a id="wf-maint-32-autofix"></a>
5) [`maint-32-autofix.yml`](../../.github/workflows/maint-32-autofix.yml) — Consolidated autofix follower
   - Triggers: `workflow_run` for the `CI` workflow (type: `completed`)
   - Jobs: `context`, `autofix`, `post-failure`
     - Evaluates opt-in labels, change volume, failure types, and automation loops before deciding to run.
     - Calls the reusable autofix action, pushes commits with `SERVICE_BOT_PAT` when eligible, or uploads patches for forks.
     - Retries trivial lint/style failures when heuristics allow and posts detailed status summaries.

<a id="wf-ci"></a>
6) [`pr-10-ci-python.yml`](../../.github/workflows/pr-10-ci-python.yml) — Test suite on pushes and PRs
   - Triggers: `push` to `phase-2-dev`, `pull_request`
   - Jobs: `core-tests` (matrix on Python 3.11/3.12), `gate` (aggregates)

<a id="wf-docker"></a>
7) [`pr-12-docker-smoke.yml`](../../.github/workflows/pr-12-docker-smoke.yml) — Build, test, and push container image
...
<a id="wf-maint-31-codex-bootstrap-cleanup"></a>
8) [`maint-31-codex-bootstrap-cleanup.yml`](../../.github/workflows/maint-31-codex-bootstrap-cleanup.yml) — Prune stale bootstrap branches
   - Triggers: `schedule` (daily), `workflow_dispatch`
   - Jobs: `prune`
     - Deletes old `agents/codex-issue-*` branches beyond TTL

<a id="wf-quarantine-ttl"></a>
9) [`maint-34-quarantine-ttl.yml`](../../.github/workflows/maint-34-quarantine-ttl.yml) — Enforce test quarantine expirations
...
10) [`agents-48-verify-service-bot-pat.yml`](../../.github/workflows/agents-48-verify-service-bot-pat.yml) — Verify `SERVICE_BOT_PAT` presence and scopes
   - Triggers: `workflow_dispatch`
   - Jobs: `check-token`
     - Checks secret exists and minimally has `repo`/`workflow` scopes

<a id="wf-copilot-readiness"></a>
11) [`agents-45-copilot-readiness.yml`](../../.github/workflows/agents-45-copilot-readiness.yml) — Copilot readiness probe
   - Triggers: `workflow_dispatch`
   - Jobs: `probe`
     - GraphQL `suggestedActors` check, temp issue assign attempt to `@copilot`, close, verdict

<a id="wf-verify-codex-bootstrap-matrix"></a>
12) [`agents-47-verify-codex-bootstrap-matrix.yml`](../../.github/workflows/agents-47-verify-codex-bootstrap-matrix.yml) — End-to-end bootstrap scenario matrix
...
13) [`maint-33-check-failure-tracker.yml`](../../.github/workflows/maint-33-check-failure-tracker.yml) — Open/close CI failure issues
...
<a id="wf-release"></a>
14) [`maint-50-release.yml`](../../.github/workflows/maint-50-release.yml) — Build/publish package and GitHub release
...
- `agents-45-copilot-readiness.yml` (Probe Copilot assignability)
...
- `agents-47-verify-codex-bootstrap-matrix.yml` (Scenario harness)
