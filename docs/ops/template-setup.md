...
- Merge manager fails with branch context error:
  - Ensure `.github/workflows/maint-46-merge-manager.yml` passes `pull-request-number` when enabling auto-merge.
- Labels not applied on fork PRs:
  - Ensure `agents-46-label-agent-prs.yml` exists and `pull_request_target` triggers are enabled.
