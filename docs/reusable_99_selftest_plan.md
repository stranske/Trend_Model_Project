...
- Replace step-level `uses` invocations in `.github/workflows/reusable-99-selftest.yml` with job-level `jobs.<id>.uses` calls targeting `.github/workflows/reusable-ci.yml`.
...
1. `.github/workflows/reusable-99-selftest.yml` dispatches reusable scenarios through a `jobs.scenario.uses` job referencing `reusable-ci.yml` with a clearly defined matrix of at least two scenarios.
...
- [x] Review `.github/workflows/reusable-ci.yml` to enumerate required inputs, outputs, and permissions for job-level reuse.
