...
1. `.github/workflows/selftest-80-reusable-ci.yml` dispatches reusable scenarios through a `jobs.scenario.uses` job referencing `reusable-10-ci-python.yml` with a clearly defined matrix of at least two scenarios.
...
- [x] Inspect current `.github/workflows/selftest-80-reusable-ci.yml` to capture existing triggers, inputs, scenario definitions, and artifact handling.
