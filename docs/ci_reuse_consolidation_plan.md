...
- Only four reusable workflows remain (`reusable-10-ci-python.yml`, `reusable-12-ci-docker.yml`, `reusable-92-autofix.yml`,
  `reusable-70-agents.yml`). The self-test matrix now lives in `.github/workflows/selftest-80-reusable-ci.yml`.
...
| Maintenance | Deleted legacy hygiene/self-test workflows (`maint-31`, `maint-34`, `maint-37`, `maint-38`, `maint-43`, `maint-44`, `maint-45`, `maint-48`, `maint-49`, `maint-52`, `maint-60`) and replaced them with a single manual self-test matrix (`selftest-80-reusable-ci.yml`). `agents-63-chatgpt-issue-sync.yml` was later reinstated (2025-10-07) to preserve issue fan-out from curated topic lists and is now guarded by tests. |
...
1. Keep the self-test matrix manual; add automation only if a new issue justifies it.
