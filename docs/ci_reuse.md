...
| ------------------ | ---- | ------- |
| Python CI          | `.github/workflows/reusable-92-ci-python.yml` | Tests + coverage gate + (optional) quarantine set |
| Autofix            | `.github/workflows/reusable-91-autofix.yml`   | Formatting / lint autofix on PRs (opt‑in label) |
...
## 1. Python CI (`reusable-92-ci-python.yml`)
...
jobs:
  core:
    uses: ./.github/workflows/reusable-92-ci-python.yml
    with:
...
## 2. Autofix (`reusable-91-autofix.yml`)
...
jobs:
  autofix:
    uses: ./.github/workflows/reusable-91-autofix.yml
    with:
...
- [ ] Introduce new consumer pointing at `reusable-92-ci-python.yml`.
