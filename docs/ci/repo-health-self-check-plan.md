...
- **Archival requirement:** Earlier remediation retired experimental self-test workflows (`selftest-83-…`, `selftest-84-…`, `selftest-88-…`, `maint-90-…`, `selftest-82-…`). Issue #2525 subsequently reinstated these scenarios under the numbered `selftest-8X-*` manual wrappers; Issue #2651 later replaced that roster with the single `selftest-runner.yml` entry point while keeping archival notes in `ARCHIVE_WORKFLOWS.md` and `docs/ci/WORKFLOWS.md`.
