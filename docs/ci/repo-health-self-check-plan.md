...
- **Archival requirement:** Retire experimental self-test workflows (legacy `maint-` wrappers and superseded `selftest-8X` variants) by moving them under `Old/` (or removing them) and document their archival location in `ARCHIVE_WORKFLOWS.md` plus `docs/ci/WORKFLOWS.md`.
