...
| **Health 40 Repo Selfcheck** | `.github/workflows/health-40-repo-selfcheck.yml` | Weekly cron (`20 6 * * 1`), `workflow_dispatch` | `contents: read`, `issues: write` | No | Summarises label coverage and branch-protection visibility, downgrading unauthorized branch checks to warnings while updating the `[health] repository self-check failed` tracker when problems persist. |
...
| `health-40-repo-selfcheck.yml` (`Health 40 Repo Selfcheck`) | `.github/workflows/health-40-repo-selfcheck.yml` | Weekly cron, manual | `contents: read`, `issues: write` | No | Repo health pulse that surfaces missing labels or branch-protection visibility gaps, warns on unauthorized branch visibility, and maintains the `[health] repository self-check failed` tracker issue. |
