...
| **Health 45 Agents Guard** | `.github/workflows/health-45-agents-guard.yml` | `pull_request_target` (agents workflow edits) | `contents: read`, `pull-requests: write` | No | Ensures guarded agents workflows change only with the allow label and CODEOWNERS approval. |
...
| **Selftest Runner** | `.github/workflows/selftest-runner.yml` | `workflow_dispatch` | `contents: read`, `actions: read`, `pull-requests: write` | No | Parameterised manual entry point for the reusable matrix (modes: summary, comment, dual-runtime) with optional PR comment output, artifact downloads, and overrides for comment/summary titles. |
...
| **Agents Critical Guard** | `.github/workflows/agents-critical-guard.yml` | `pull_request` | Defaults (`contents: read`) | No | Blocks deletions or renames of protected agents workflows by diffing the base branch and enforcing `.github/scripts/ensure_agents_workflows_intact.py`. |
...
| `health-45-agents-guard.yml` (`Health 45 Agents Guard`) | `.github/workflows/health-45-agents-guard.yml` | `pull_request_target` for agents workflow edits | `contents: read`, `pull-requests: write` | No | Blocks unlabelled or unapproved changes to guarded agents workflows by requiring the allow label and CODEOWNERS review. |
...
| `selftest-runner.yml` (`Selftest Runner`) | Single manual wrapper that toggles summary vs PR comment output and the Python version matrix via `mode` inputs. Optional fields (`pull_request_number`, `summary_title`, `comment_title`, `reason`) fine-tune comment delivery and run summaries. |
