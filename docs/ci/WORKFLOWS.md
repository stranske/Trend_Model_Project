...
| --- | --- | --- | --- | --- | --- |
| **Gate** | `.github/workflows/pr-gate.yml` | Fan-out orchestrator that reuses the Python CI matrix and Docker smoke reusable workflows, then enforces all downstream results. | `pull_request` (non-doc paths) and `workflow_dispatch`. | Uses default `GITHUB_TOKEN`; delegated `reusable-ci.yml` optionally consumes the `pypi-token` secret, and the Docker reusable workflow needs `packages: write` to push on `phase-2-dev`. | Publishes status checks for `core tests (3.11)`, `core tests (3.12)`, `docker smoke`, and the summary `gate` job. No labels are applied. |
| **PR 10 CI Python** | `.github/workflows/pr-10-ci-python.yml` | Primary style/type/test runner calling `reusable-96-ci-lite.yml` with coverage enforcement. | `pull_request`, `push` to `phase-2-dev`, `workflow_dispatch`, and `workflow_call`. | Relies on the `GITHUB_TOKEN` plus optional `pypi-token` when invoked through `workflow_call`; inherits read-only permissions from the reusable workflow. | Emits a single `ci / python` status check (version noted in parentheses) and does not manage labels. |
| **PR 12 Docker Smoke** | `.github/workflows/pr-12-docker-smoke.yml` | Lints the Dockerfile, builds a cached image, runs container health checks, and executes the bundled pytest suite. | `workflow_call` (used by Gate and other orchestrators) and `workflow_dispatch`. | Requires `packages: write` so pushes from `phase-2-dev` can publish to `${{ vars.REGISTRY || ghcr.io }}`; otherwise uses the default `GITHUB_TOKEN`. | Surfaces `Lint Dockerfile` and `smoke` status checks. Does not touch labels. |
| **Autofix** | `.github/workflows/autofix.yml` | Lightweight formatting/type hygiene runner that auto-commits fixes for same-repo PRs or uploads a patch for forks. | `pull_request` (including label changes). | Needs `contents: write` and `pull-requests: write`; inherits secrets for the reusable autofix composite (optional `service_bot_pat`). | Top-level `apply` → `autofix` job status. Labels ensured/applied: `autofix`, `autofix:applied` or `autofix:patch`, and mutually exclusive `autofix:clean` / `autofix:debt` based on residual lint. |
| **Maint 02 Repo Health** | `.github/workflows/maint-02-repo-health.yml` | Weekly sweep that summarises stale branches and unassigned issues in the run summary. | Monday cron (`15 7 * * 1`) plus `workflow_dispatch`. | `contents: read`, `issues: read`; no additional secrets. | Single `Weekly repository health sweep` status with no label mutations. |
| **Agents Consumer** | `.github/workflows/agents-consumer.yml` | Hourly/adhoc entry point that parses JSON input then dispatches to the reusable agents toolkit for readiness, bootstrap, watchdog, and keepalive tasks. | Hourly cron (`15 * * * *`) and `workflow_dispatch` with `params_json`. | Requires `contents`, `pull-requests`, and `issues` write; forwards optional `service_bot_pat` to downstream jobs. | Posts `Resolve Parameters` and `Dispatch Agents Toolkit` checks; downstream `reusable-70-agents` jobs surface `Agent Readiness Probe`, `Codex Preflight`, `Bootstrap Codex PRs`, `Agent Watchdog`, and keepalive statuses when enabled. Bootstrap runs assign Codex actors and add the `agent:codex` label to spawned PRs. |
| **Reuse Agents** | `.github/workflows/reuse-agents.yml` | Workflow-call wrapper so other repositories or orchestrators can invoke the agents toolkit with consistent inputs. | `workflow_call` only. | Requires the same write permissions as Agents Consumer and optionally a `service_bot_pat` secret for Codex bootstrap. | Exposes the `call` job, which fans out into the reusable agents checks listed above; label side effects mirror `codex-bootstrap-lite` (e.g., `agent:codex` on created PRs). |
