...
| Workflow | Trigger(s) | Purpose |
|----------|------------|---------|
| `agents-70-orchestrator.yml` (`Agents 70 Orchestrator`) | 20-minute cron, manual | Single entry point that delegates readiness, preflight, verification, bootstrap, keepalive, and diagnostics to the reusable agents workflow.
