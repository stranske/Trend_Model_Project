...
| **Fund selector Playwright smoke** | `.github/workflows/pr-12-playwright.yml` | `push`/`pull_request` targeting `phase-2-dev` + `main`, `workflow_dispatch` | `contents: read` | **No** – supplemental E2E check. | Runs a small Playwright smoke against the Streamlit fund selector so UI regressions are caught early. |
...
* [`Fund selector Playwright smoke`](../../.github/workflows/pr-12-playwright.yml) runs a Playwright E2E smoke for the fund selection flow on pushes and PRs targeting `phase-2-dev` and `main`.
