...
> ✅ **Required checks.** Branch protection now requires both Gate and Health 45 Agents Guard. Gate aggregates the CI matrix, while the guard runs on every PR and blocks only when protected agents workflows are deleted, renamed, or edited without the allow label + CODEOWNER approval.
...
| **Health 45 Agents Guard** | `.github/workflows/health-45-agents-guard.yml` | `pull_request_target` (runs on every PR) | `contents: read`, `pull-requests: write` | **Yes** – required status check that blocks protected agents workflow edits without label + approval. | Ensures guarded agents workflows change only with the allow label and CODEOWNERS approval. |
...
| `health-45-agents-guard.yml` (`Health 45 Agents Guard`) | `.github/workflows/health-45-agents-guard.yml` | `pull_request_target` (runs on every PR) | `contents: read`, `pull-requests: write` | **Yes** | Blocks unlabelled or unapproved changes to guarded agents workflows by requiring the allow label and CODEOWNERS review. |
