# agents-consumer.yml Repair / Decommission Plan

## Scope & Key Constraints
- **Workflow coverage** – Focus on `.github/workflows/agents-consumer.yml` and the reusable it calls (`reusable-70-agents.yml`). Do not alter orchestrator (`agents-70-orchestrator.yml`) wiring unless a contract change is required by the repair.
...
1. **Successful dispatch** – Manual `workflow_dispatch` runs of `agents-consumer.yml` complete without schema errors. If downgraded, only `workflow_dispatch` remains as a trigger.
...
3. **Consumer wiring** – `agents-consumer.yml` invokes the reusable using a `jobs.<name>.uses: ./.github/workflows/reusable-70-agents.yml` block, forwarding `with:` parameters and `secrets: inherit` (or explicit mapping) as needed.
4. **Concurrency guard** – A `concurrency:` block exists on the consumer workflow referencing a stable group name (e.g., `agents-consumer`).
...
- [ ] Inspect current Actions run failure logs for `.github/workflows/agents-consumer.yml` to capture exact schema complaints.
...
- [ ] Rewrite `agents-consumer.yml` jobs section to use the reusable via `uses` syntax, forwarding `with:` parameters and `secrets: inherit`.
