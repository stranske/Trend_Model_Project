...
- **Workflow coverage** – Focus on `.github/workflows/agents-61-consumer-compat.yml` and the reusable it calls (`reusable-16-agents.yml`). Do not alter orchestrator (`agents-70-orchestrator.yml`) wiring unless a contract change is required by the repair.
...
2. **Reusable contract** – `.github/workflows/reusable-16-agents.yml` contains a valid `on: workflow_call` definition with named `inputs` (and defaults where appropriate). Inputs/secrets consumed inside the reusable match those forwarded by the consumer.
3. **Consumer wiring** – `agents-61-consumer-compat.yml` invokes the reusable using a `jobs.<name>.uses: ./.github/workflows/reusable-16-agents.yml` block, forwarding `with:` parameters and `secrets: inherit` (or explicit mapping) as needed.
...
- [ ] Audit `.github/workflows/reusable-16-agents.yml` to confirm or add `workflow_call` definition and enumerate required inputs/secrets.
