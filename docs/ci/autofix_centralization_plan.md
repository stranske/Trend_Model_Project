...
- For the former pre-CI `pr-02-autofix` workflow, enforce opt-in behaviour via an `autofix:clean` label and guarantee only one in-flight run per pull request by using GitHub Actions concurrency controls. *(Superseded by Maint 46 Post CI.)*
