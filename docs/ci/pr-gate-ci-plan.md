...
- **Workflows affected**: `.github/workflows/pr-gate.yml`, `.github/workflows/reusable-ci.yml`,
  `.github/workflows/reusable-docker.yml`, and the maintenance listeners that react to CI completion
   (`maint-post-ci.yml`, `maint-33-check-failure-tracker.yml`).
...
3. Maintenance followers (`maint-post-ci.yml`, `maint-33-check-failure-tracker.yml`) subscribe to
...
- [x] Point `maint-post-ci.yml` and `maint-33-check-failure-tracker.yml` at Gate.
