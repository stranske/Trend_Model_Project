...
- Ensure all workflow names, file paths, and labels reflect the current repository configuration (Gate required check, consolidated Maint Post-CI, orchestrator/consumer workflows, and reuse includes).
...
- Section explicitly calls out:
  ...
  - How agent automations (bootstrap + consumer) are triggered, including the relevant labels and their effects.
  - Links to the workflow YAML files: `agents/agents-70-orchestrator.yml`, `agents/agents-consumer.yml`, and `reuse-agents.yml`.
