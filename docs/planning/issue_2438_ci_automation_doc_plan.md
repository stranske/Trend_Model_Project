...
- Ensure all workflow names, file paths, and labels reflect the current repository configuration (Gate required check, consolidated Gate summary job, the Agents 70 Orchestrator workflow, and reusable includes).
...
- Section explicitly calls out:
  ...
  - Note that PR coverage/status are summarized in a single consolidated Gate summary job comment.
...
2. [x] Audit the latest workflow files to verify names, labels, and behaviors (Gate, Gate summary job, orchestrator, reusable includes, autofix opt-in/out).
3. [x] Draft the new section content covering required points (Gate, autofix timing & opt-in/out, agent automation triggers, consolidated Gate summary job summary).
