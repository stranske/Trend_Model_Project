...
- Normalize all consumers to invoke a single reusable workflow entrypoint (`.github/workflows/reusable-70-agents.yml`).
...
1. `.github/workflows/reusable-70-agents.yml` exposes a `workflow_call` trigger with documented inputs and defaults that cover all current consumer needs.
...
3. All wrapper workflows reference only `.github/workflows/reusable-70-agents.yml`; obsolete or duplicate reusable workflow files are removed or deprecated.
