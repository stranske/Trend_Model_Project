...
- Reusable workflow file exists (e.g., `.github/workflows/agents-44-verify-agent-assignment.yml`) exposing a `workflow_call` interface with a required `issue_number` input.
...
- Added `.github/workflows/agents-44-verify-agent-assignment.yml`, a reusable and dispatchable workflow that consumes an `issue_number`,
