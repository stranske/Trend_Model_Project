...
Issue labeled agent:codex    ──▶ agents-41-assign.yml (wrapper)
                ...
                agents-42-watchdog.yml (wrapper) ──▶ delegates manual watchdog runs when needed
                              │
PR opened/updated            ──▶ pr-02-label-agent-prs.yml (pull_request_target) → idempotent labeling
                              │
PR (any)                     ──▶ maint-32-autofix.yml (workflow_run follower) → composite autofix action
                              │
Failing CI / Docker / Tests  ──▶ maint-32-autofix.yml (workflow_run follower) → composite autofix action
```
...
### 6. `maint-38-cleanup-codex-bootstrap.yml`
