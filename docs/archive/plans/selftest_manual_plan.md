...
> the nightly cron on `selftest-reusable-ci.yml` once the reusable wrapper was retired.
...
- Limit automation to the GitHub Actions workflow `selftest-reusable-ci.yml` (the successor to the `selftest-8X-*` wrappers).
...
1. The remaining self-test workflow (`selftest-reusable-ci.yml`) defines **only** a `workflow_dispatch` trigger, optionally with a `reason` input, and has no other automatic triggers (`push`, `pull_request`, `schedule`, etc.). ✅ Verified for the runner during the manual-only phase (superseded when the nightly cron returned in Issue #2728).
...
- [x] Inventory the self-test workflow file under `.github/workflows/` (`selftest-reusable-ci.yml`) and confirm current triggers.
