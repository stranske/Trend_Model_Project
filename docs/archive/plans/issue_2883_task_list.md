...
- Limit changes to the Gate summary job, the reusable autofix composite, supporting scripts, and associated documentation/tests.
...
## Work plan
- [x] Inventory current workflows to confirm the Gate summary job is the sole autofix entry point and identify any legacy files to remove.
- [x] Refine the Gate summary job to gate reusable autofix invocations on the `autofix:clean` opt-in label and enforce dry-run handling for forks.
...
> ✅ Acceptance criteria satisfied — The Gate summary job is now the lone opt-in autofix path, label-gated, and exercises a dry-run-safe reusable workflow with comprehensive reporting.
