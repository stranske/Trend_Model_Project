...
The metrics output is printed to the console and can also be written to XLSX, CSV, or JSON according to `export.formats` in the config.
...
- Missing-data behaviour is configured per asset via `data.missing_policy` (`drop`, `ffill` or `zero`). When forward-fill is selected the optional `data.missing_limit` caps the length of consecutive gaps that will be filled.
...
| `vol_adjust.floor_vol` | float | Minimum annualised volatility per asset when forming the inverse-volatility tilt, limiting concentration in very low-volatility assets. This long-only engine has no leverage path. |
...
List `xlsx`, `csv`, or `json` under `export.formats`. XLSX exports include a formatted summary sheet generated by `trend_analysis.export.make_summary_formatter` and can be inspected with any spreadsheet program.
...
|-----|------|---------|-------------|
| `long_only` | bool | `true` | Clip negative weights then renormalise. Applies to `custom_weights` or plugin engines that emit shorts; built-in schemes remain non-negative. |
