...
| --- | --- | --- |
| Valid instruction comment (allowed author, hidden markers) triggers PR-meta run reporting `ok: true`, `reason: keepalive-detected`, and populated round/trace/PR fields. | ❌ Not satisfied | Latest detector runs `Agents PR meta manager` [#19087372965](https://github.com/stranske/Trend_Model_Project/actions/runs/19087372965) (`pull_request`) and [#19087550353](https://github.com/stranske/Trend_Model_Project/actions/runs/19087550353) (`push`) both skipped the `Detect keepalive round comments` job, leaving `ok`/`reason` unset. |
| Exactly one orchestrator workflow_dispatch fires with matching TRACE and no cancellations triggered by other keepalive rounds. | ❌ Not satisfied | Orchestrator run [#19087550223](https://github.com/stranske/Trend_Model_Project/actions/runs/19087550223) terminated before any jobs executed, so no TRACE propagation or dispatch occurred. |
| Exactly one repository_dispatch (`codex-pr-comment-command`) emitted per accepted instruction comment. | ❌ Not satisfied | Detector run [#19087550353](https://github.com/stranske/Trend_Model_Project/actions/runs/19087550353) never reached dispatch steps; no repository_dispatch events were recorded. |
| Guard failures yield one-line PR comment `Keepalive {round} {trace} skipped: <reason>` and matching summary entry. | ❌ Not satisfied | Orchestrator run [#19087550223](https://github.com/stranske/Trend_Model_Project/actions/runs/19087550223) aborted pre-job, so skip comment and summary rows were never produced. |
| Two consecutive valid rounds produce distinct traces, distinct orchestrator runs, and no duplicate dispatches. | ❌ Not satisfied | Recent detector/orchestrator runs ([#19087372965](https://github.com/stranske/Trend_Model_Project/actions/runs/19087372965), [#19087550353](https://github.com/stranske/Trend_Model_Project/actions/runs/19087550353), [#19087550223](https://github.com/stranske/Trend_Model_Project/actions/runs/19087550223)) all failed prior criteria, so no qualifying consecutive rounds exist yet. |
