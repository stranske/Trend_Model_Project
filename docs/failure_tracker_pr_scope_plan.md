...
- Operate only on repositories where the Gate follower (`maint-46-post-ci.yml`) is configured; legacy `maint-47-check-failure-tracker.yml` shells simply delegate.
...
- [x] Audit the historical `maint-47-check-failure-tracker.yml` triggers and add guard clauses to ensure the job exits early unless the source event is a PR.
