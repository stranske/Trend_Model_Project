# Keepalive Evaluation – PR 3429 (2025-11-10)
...
| --- | --- | --- |
| `agents:keepalive` label present when instructions post | Timeline shows label removal at 07:07:42Z (`unlabeled` event by `stranske`) while Rounds 11–13 landed between 07:08Z and 07:11Z (`tmp_logs/pr3429_timeline.json`). | ❌ Violated |
| Human kickoff via @mention | Round 2 instruction (`06:52:48Z`) authored by `stranske` directly pings `@codex` with the task list. | ✅ |
| Gate success at current head | Instruction comments (e.g. Round 10 and Round 19) include the “Latest Runs: ✅ success — Gate” block generated by the orchestrator. | ✅ |

**Impact:** Guardrail labels disappeared for ~6h, yet automation continued posting instructions, breaching Goals §1.

## 2. Repeat Contract (Goals & Plumbing §2)

- Orchestrator reruns after Round 10 ignored the missing label and kept emitting instructions (Rounds 11–13).
- No evidence of a compensating pause or halt was observed.

❌ Overall – repeat guardrails are not enforced.
...
- Despite the critical label gap (07:07Z–13:34Z), automation did not pause rounds.

❌ Controls failed to halt activity when guardrails broke.

## 5. No-Noise Policy (Goals & Plumbing §5)

Twenty-four keepalive instructions (plus duplicate Round 5 entries) posted in ~7 hours, including three rounds while guardrails were invalid. The volume and timing constitute contract noise.

⚠️ Partially satisfied – instructions are well-formed, but cadence becomes spam once prerequisites fail.
...
- Scheduled runs pile up without respecting the run cap (see §3).
...
⚠️ Pending – automation restarts but never converges.

## Connector Payload Contract (Sync Checklist)

- No evidence that `update-branch` or `create-pr` payloads were accepted; branch never advanced and no connector PR was opened. | ❌

## Pause Label Clearance

- `agents:sync-required` was never applied, so no clearance occurred. | ⚠️ Missing escalation.
...
- Timeline gap: `agents:keepalive` absent 07:07Z–13:34Z while three rounds continued to post.
