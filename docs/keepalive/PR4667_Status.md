...
Checklist reconciled on 2026-02-04 after reviewing commits `14cb91c8` and `240272c3` and running `pytest tests/monte_carlo/test_runner_turnover_diagnostics.py -m "not slow"`.
...
- [ ] Develop unit tests to confirm that regime-conditional turnover caps are correctly implemented and affect runner behavior.
...
- [ ] Review and enhance redaction in the Streamlit NL operation viewer to ensure replay functionality does not leak sensitive data.
...
- [ ] Unit tests confirm that regime-conditional turnover caps are accepted through the configuration, correctly interpreted, and affect runner behavior as expected.
