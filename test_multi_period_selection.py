...
def test_multi_period_selection():
    ...
    for i in range(min(3, len(results))):
        ...
        if in_stats:
            ...
            sharpe_data = [(mgr, getattr(stats, 'sharpe', 0)) for mgr, stats in in_stats.items() 
    ...
    if len(results) > 1:
        unique_selections = len(set(tuple(sorted(period_selections.get(i, []))) 
                                     for i in range(len(results))))
        if unique_selections > 1:
            checks.append(f"✅ Selection varies across periods ({unique_selections} unique combinations)")
        else:
