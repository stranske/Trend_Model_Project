...
- **Issue #2814 consolidation (2026-??-??)**: `selftest-runner.yml` was superseded by `selftest-reusable-ci.yml`, restoring the canonical workflow name “Selftest: Reusables,” keeping the nightly cron at `30 6 * * *`, and trimming the run summary down to a compact matrix table in `GITHUB_STEP_SUMMARY`.
