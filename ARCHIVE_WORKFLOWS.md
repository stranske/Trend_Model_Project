...
- **2025-10-19 disablement (Issue #2728)**: Legacy workflow entries (`selftest-pr-comment.yml`, `maint-43-selftest-pr-comment.yml`, `maint-44-selftest-reusable-ci.yml`, `maint-48-selftest-reusable-ci.yml`, `pr-20-selftest-pr-comment.yml`) were manually disabled via `gh workflow disable` so the Actions UI now lists only **Selftest Runner**.
