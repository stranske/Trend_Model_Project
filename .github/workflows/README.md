...
- Agents intake + orchestration (`agents-63-issue-intake.yml` + wrappers, `agents-70-orchestrator.yml`, `reusable-16-agents.yml`): unified entry point for ChatGPT topic imports and Codex readiness/bridge sweeps. Use the [Agent task issue template][agent-task-template] (auto-labels `agents` + `agent:codex`) to raise work for Codex; the thin wrappers forward to the intake workflow so parsing and bridge logic stay centralised.
...
| `agents-63-chatgpt-issue-sync.yml` | workflow_dispatch | Manual wrapper that forwards curated topic lists into `agents-63-issue-intake.yml`. |
| `agents-63-issue-intake.yml` | workflow_call | Shared intake workflow consumed by the ChatGPT sync and Codex bridge wrappers. |
...
| `agents-63-codex-issue-bridge.yml` | issues, workflow_dispatch | Label-triggered wrapper that routes Codex issues into `agents-63-issue-intake.yml`. |
