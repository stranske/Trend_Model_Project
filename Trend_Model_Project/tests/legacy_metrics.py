...
@register_metric("zscore_ret")
def zscore_ret(prices: pd.DataFrame, cfg: RiskStatsConfig) -> pd.Series:
    returns = _ann_return(prices, cfg)  # ← existing helper
    return (returns - returns.mean()) / returns.std(ddof=0)


@register_metric("info_ratio")
def information_ratio(prices: pd.DataFrame, cfg: RiskStatsConfig) -> pd.Series:
    ret = _ann_return(prices, cfg)
    bench = _load_benchmark(prices.index)  # existing util, or pass via cfg
    excess = ret - bench
