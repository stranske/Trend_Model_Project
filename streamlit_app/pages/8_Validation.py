...
TESTABLE_SETTINGS: list[SettingDef] = [
    ...
    SettingDef(
        key="vol_adjust_enabled",
        label="Volatility Adjustment Enabled",
        category="Risk",
        baseline=True,
        test_values=[False],
        expected_effect="Volatility scaling on weights",
        economic_intuition="Disabling adjustment removes volatility-based scaling",
    ),
    SettingDef(
        key="vol_window_length",
        label="Volatility Window Length",
        category="Risk",
        baseline=63,
        test_values=[21, 126],
        expected_effect="Volatility estimate responsiveness",
        economic_intuition="Shorter windows react faster to recent volatility",
    ),
    SettingDef(
        key="vol_window_decay",
        label="Volatility Window Decay",
        category="Risk",
        baseline="ewma",
        test_values=["simple"],
        expected_effect="Volatility weighting profile",
        economic_intuition="Decay choice shifts how recent data is weighted",
    ),
    SettingDef(
        key="vol_ewma_lambda",
        label="EWMA Lambda",
        category="Risk",
        baseline=0.94,
        test_values=[0.8, 0.97],
        expected_effect="Volatility smoothing intensity",
        economic_intuition="Lower lambda increases responsiveness to recent moves",
    ),
    SettingDef(
        key="vol_floor",
