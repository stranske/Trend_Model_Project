...
def test_export_aggregation_results_drops_parquet_when_unavailable(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(export_module, "_supports_parquet", lambda: False)
    exported = export_aggregation_results(aggregation, tmp_path, formats=["csv", "parquet"])

    assert exported["path_summary_csv"].exists()
    assert "path_summary_parquet" not in exported


def test_export_aggregation_results_rejects_parquet_only_when_unavailable(
    tmp_path, monkeypatch
) -> None:
    results_frame = _sample_results_frame()
    aggregation = aggregate_monte_carlo_results(
...
def test_export_aggregation_results_uses_path_hash(tmp_path) -> None:
    ...
    assert "path_hash" not in path_summary.columns
