...
| 2 | `regime.enabled` toggle is a no-op in the demo fixture | **Fixture gap** — scalar max_turnover only bypasses regime-specific cap lookup for the turnover cap. The demo fixture does not yet exercise a regime-conditional turnover-cap mapping, so this toggle remains report-only until a regime-conditional scenario is added. |
...
| 7 | `portfolio.weighting.name` (`risk_parity`) was a no-op in the demo fixture | **FIXED 2026-06-11 (#5537)** — the `weighting_risk_parity` scenario inherited the demo fixture's pinned `portfolio.custom_weights`, which takes precedence: the weight engine only runs when `custom_weights is None` (`src/trend_analysis/stages/portfolio.py:346`). Clearing `custom_weights` in the scenario base makes equal vs risk_parity diverge, and the scenario is enforced (`tests/baseline/catalog.yaml:93-107`). Not a wiring gap — a fixture-precedence artifact. |
| 8 | `portfolio.robustness.shrinkage.enabled` toggle is a no-op in the default demo fixture | **FIXED 2026-06-11 (#5538)** — the default demo uses equal/manual weighting, so shrinkage cannot affect output. The Tier-2 toggle runs against `config/robust_demo.yml`, whose `portfolio.weighting.name: robust_mv` delegates robust-engine parameters to `portfolio.robustness`; shrinkage on/off changes allocation and is enforced. |
