...
| 2 | `regime.enabled` toggle is a no-op in the demo fixture | **Fixture gap** — scalar max_turnover only bypasses regime-specific cap lookup; in the current multi-period engine path this setup is allocation-neutral for `regime.enabled`. The demo fixture does not yet exercise a regime-conditional turnover-cap mapping, so this toggle remains report-only until a regime-conditional scenario is added. |
