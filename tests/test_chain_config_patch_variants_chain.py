"""Tests for ConfigPatchVariantsChain retry prompts."""
...
def test_retry_prompt_uses_configpatchvariants_schema_note() -> None:
    patch_payload = {
        "operations": [],
        "risk_flags": [],
        "summary": "Ok",
    ...
    payload = {
        "variants": [
            {"label": "conservative", "patch": patch_payload},
            {"label": "baseline", "patch": patch_payload},
            {"label": "aggressive", "patch": patch_payload},
        ]
    }
    ...
    assert "ConfigPatchVariants schema" in llm.prompts[1]
