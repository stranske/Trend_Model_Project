...
def test_diagnostic_result_failure_wraps_payload():
    ...
    result = DiagnosticResult.failure(
        reason_code="PIPELINE_MISSING_DATA", message="unusable", context=context
    )

    diagnostic = result.diagnostic
    assert result.value is None
    assert diagnostic is not None
    assert diagnostic.reason_code == "PIPELINE_MISSING_DATA"
    assert diagnostic.message == "unusable"
...
def test_coerce_pipeline_result_preserves_diagnostic_when_payload_missing():
    diagnostic = DiagnosticPayload(
        diagnostics.PipelineReasonCode.INPUT_NONE.value, "missing payload"
    )
    ...
    payload, coerced_diag = diagnostics.coerce_pipeline_result(Wrapper())

    assert payload is None
    assert coerced_diag is diagnostic


def test_coerce_pipeline_result_with_pipeline_result_round_trip():
    failure = diagnostics.pipeline_failure(
        diagnostics.PipelineReasonCode.NO_VALUE_COLUMNS,
        context={"columns": []},
    )

    payload, diagnostic = diagnostics.coerce_pipeline_result(failure)

    assert payload is None
    assert diagnostic is failure.diagnostic


def test_pipeline_failure_without_context_sets_none():
    result = diagnostics.pipeline_failure(diagnostics.PipelineReasonCode.INPUT_NONE)
