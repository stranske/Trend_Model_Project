...
def _build_config(csv_path: str, export_dir: str | None = None) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        export={"directory": export_dir or "outputs", "formats": ["json"], "filename": "report"},
    )
...
def test_main_happy_path(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
    ...
    monkeypatch.setattr(run_analysis.export, "make_summary_formatter", lambda *_, **__: lambda df: df)

    exit_code = run_analysis.main(["-c", "config.yml"])

    assert exit_code == 0
    assert export_calls["formats"] == ["json"]
    assert export_calls["payload_keys"] == {"metrics", "performance_by_regime", "regime_notes"}


def test_main_raises_for_missing_csv_path(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(run_analysis, "load", lambda _: SimpleNamespace(data={}, export={}, sample_split={}))
    with pytest.raises(KeyError):
        run_analysis.main(["--config", "config.yml"])


def test_main_raises_when_load_csv_returns_none(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
...
def test_main_uses_nan_policy_fallbacks(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
    cfg = SimpleNamespace(
        ...
        export={"directory": str(tmp_path / "out"), "formats": ["json"], "filename": "report"},
    )

    captured_kwargs: dict[str, Any] = {}

    def _load_csv(path: str, *, errors=None, nan_policy=None, nan_limit=None):
        captured_kwargs.update({"path": path, "errors": errors, "nan_policy": nan_policy, "nan_limit": nan_limit})
...
def test_main_detailed_mode_handles_empty_metrics(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
    ...
    monkeypatch.setattr(run_analysis, "load", lambda _: cfg)
    monkeypatch.setattr(run_analysis, "load_csv", _load_csv)
    monkeypatch.setattr(run_analysis, "api", SimpleNamespace(run_simulation=lambda *_: empty_result))
...
def test_main_applies_default_export_settings(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
    cfg = SimpleNamespace(
        data={"csv_path": tmp_path / "data.csv", "missing_policy": "drop", "missing_limit": 0.1},
        sample_split={"in_start": "start", "in_end": "mid", "out_start": "mid2", "out_end": "end"},
        export={},
    )
