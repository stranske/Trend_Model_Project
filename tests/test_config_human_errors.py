...
class TestHumanErrors:
    ...
    def test_empty_version(self):
        ...
        cfg = {"version": "", "data": {}}
        with pytest.raises((ValidationException, ValueError), match="String should have at least 1 character"):
            config.load_config(cfg)

    def test_whitespace_only_version(self):
        """Test whitespace-only version."""
        cfg = {"version": "   ", "data": {}}
        with pytest.raises((ValidationException, ValueError), match="Version field cannot be empty"):
            config.load_config(cfg)

    def test_data_as_string(self):
        """Test data section as string instead of dict."""
        cfg = {"version": "1.0", "data": "not a dictionary"}
        with pytest.raises((ValidationException, ValueError), match="data must be a dictionary"):
            config.load_config(cfg)
    ...
    def test_preprocessing_as_list(self):
        ...
        with pytest.raises((ValidationException, ValueError), match="preprocessing must be a dictionary"):
            config.load_config(cfg)

    def test_vol_adjust_as_number(self):
        """Test vol_adjust section as number instead of dict."""
        cfg = {"version": "1.0", "data": {}, "vol_adjust": 42}
        with pytest.raises((ValidationException, ValueError), match="vol_adjust must be a dictionary"):
            config.load_config(cfg)

    def test_portfolio_as_string(self):
        """Test portfolio section as string instead of dict."""
        cfg = {"version": "1.0", "data": {}, "portfolio": "invalid"}
        with pytest.raises((ValidationException, ValueError), match="portfolio must be a dictionary"):
            config.load_config(cfg)

    @pytest.mark.parametrize("section", [
        "data", "preprocessing", "vol_adjust", "sample_split", 
        "portfolio", "metrics", "export", "run"
    ])
    def test_section_as_null(self, section):
        """Test required sections as null."""
        cfg = {"version": "1.0", "data": {}}
        cfg[section] = None
        with pytest.raises((ValidationException, ValueError), match=f"{section} must be a dictionary"):
            config.load_config(cfg)
    ...
    def test_yaml_file_with_syntax_error(self):
        ...
        try:
            with pytest.raises(Exception):  # YAML parsing will fail
