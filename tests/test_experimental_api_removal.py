...
def test_health_wrapper_has_specific_exceptions():
    ...
    assert "(ImportError, ModuleNotFoundError)" in content, "Should catch specific import exceptions"
    
    # Check for specific app creation exceptions (may be formatted across multiple lines)
    assert "ImportError," in content and "AttributeError," in content and "TypeError," in content, \
        "Should catch specific app creation exceptions"
    
    # Should not use bare Exception (except in comments)
    lines = content.split('\n')
    code_lines = [line for line in lines if not line.strip().startswith('#') and 'except Exception' in line]
