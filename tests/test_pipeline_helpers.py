...
def test_policy_from_config_merges_defaults() -> None:
    spec = {"policy": "drop", "per_asset": {"A": "zero"}, "limit": 5, "per_asset_limit": {"A": 2}}
...
def test_derive_split_from_periods_handles_ratio_and_date() -> None:
    ...
    result = _derive_split_from_periods(periods, method="date", boundary=pd.Period("2020-03", freq="M"), ratio=0.5)
...
def test_resolve_sample_split_validates_dataframe() -> None:
    df = pd.DataFrame({"Date": pd.date_range("2020-01-31", periods=4, freq="M"), "Fund": [0.1, 0.2, 0.3, 0.4]})
