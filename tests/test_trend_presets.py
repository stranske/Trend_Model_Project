...
@pytest.fixture
def preset_environment(tmp_path: Any, monkeypatch: pytest.MonkeyPatch) -> tuple[Any, Any]:
...
def test_preset_registry_honours_precedence(
    ...
    _write_yaml(
        env_dir / "alpha.yml",
        {
            ...
            "vol_adjust": {"enabled": False, "target_vol": 0.15, "window": {"length": 30}},
        },
    )
...
def test_get_trend_preset_supports_label_lookup(preset_environment: tuple[Any, Any]) -> None:
...
def test_get_trend_preset_raises_for_unknown(preset_environment: tuple[Any, Any]) -> None:
    base_dir, _ = preset_environment
    _write_yaml(
        base_dir / "zeta.yml",
        {
...
def test_apply_trend_preset_merges_config(preset_environment: tuple[Any, Any]) -> None:
    ...
    config = DummyConfig(signals={"existing": True}, vol_adjust={"enabled": False}, run={})
...
def test_apply_trend_preset_handles_non_mapping_attributes(preset_environment: tuple[Any, Any]) -> None:
    base_dir, _ = preset_environment
...
def test_apply_trend_preset_handles_mapping_proxy_attributes(preset_environment: tuple[Any, Any]) -> None:
    base_dir, _ = preset_environment
...
def test_helper_functions_cover_edge_cases() -> None:
    weights = presets._normalise_metric_weights({"Sharpe_Ratio": "2", "sharpe": "bad", "unknown": 1})
...
def test_trend_preset_defaults_with_sparse_config() -> None:
    spec = TrendSpec(window=45, min_periods=30, lag=3, vol_adjust=True, vol_target=None, zscore=True)
...
def test_signals_mapping_includes_optional_fields() -> None:
    spec = TrendSpec(window=25, min_periods=10, lag=2, vol_adjust=True, vol_target=0.3, zscore=False)
    preset = presets.TrendPreset(
        slug="sig",
        label="Signals",
        description="",
...
def test_vol_adjust_defaults_prefers_spec_when_section_present() -> None:
    spec = TrendSpec(window=80, min_periods=None, lag=1, vol_adjust=True, vol_target=0.4, zscore=False)
...
def test_vol_adjust_defaults_uses_spec_when_no_overrides() -> None:
    spec = TrendSpec(window=90, min_periods=None, lag=1, vol_adjust=True, vol_target=None, zscore=False)
    preset = presets.TrendPreset(
        slug="no-override",
        label="No Override",
        description="",
...
def test_vol_adjust_defaults_handles_readonly_window(monkeypatch: pytest.MonkeyPatch) -> None:
    spec = TrendSpec(window=70, min_periods=None, lag=1, vol_adjust=True, vol_target=None, zscore=False)
    raw_config = MappingProxyType({"vol_adjust": {"window": MappingProxyType({"length": None})}})
    preset = presets.TrendPreset(
        slug="readonly",
        label="Readonly",
        description="",
...
def test_vol_adjust_defaults_respects_existing_configuration(preset_environment: tuple[Any, Any]) -> None:
    base_dir, _ = preset_environment
    ...
    assert overrides["target_vol"] == presets._DEFAULT_VOL_ADJUST["target_vol"]
    assert overrides["window"]["length"] == presets._DEFAULT_VOL_ADJUST["window"]["length"]


def test_candidate_preset_dirs_deduplicates(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
...
def test_candidate_preset_dirs_includes_defaults(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
