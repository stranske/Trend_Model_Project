...
def demo_multi_period_integration():
    ...
    for i, period in enumerate(periods):
        ...
        score_frames[period] = pd.DataFrame(
            {"Sharpe": scores, "CAGR": scores + 0.05, "MaxDrawdown": -scores * 0.5},
            index=funds,
        )
    ...
    print(
        f"Average cost per period: {portfolio.total_rebalance_costs / len(periods):.4f}"
    )
...
def demo_edge_cases():
    ...
    strategy = TurnoverCapStrategy(
        {"max_turnover": 0.1, "cost_bps": 15, "priority": "largest_gap"}
    )
    ...
    print(
        f"Final allocation: {dict((k, round(v, 3)) for k, v in new_weights[new_weights > 0.001].items())}"
    )
