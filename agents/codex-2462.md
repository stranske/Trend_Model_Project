...
- Touch only the orchestration workflows involved in delegating to `reusable-16-agents.yml` (the reusable workflow itself and its orchestrator/consumer callers) plus related documentation (`docs/ci_reuse.md`).
...
- Reintroduce equivalent or tighter per-job timeouts inside `reusable-16-agents.yml`, keeping each long-running job within a 15–30 minute ceiling and preserving existing behaviour.
...
- `.github/workflows/reusable-16-agents.yml` validates without “Unexpected value 'timeout-minutes'” or other structural errors when linted or parsed by GitHub Actions.
- `.github/workflows/reusable-16-agents.yml` defines explicit `timeout-minutes` values for every long-running job (readiness probe, preflight, diagnostic/bootstrap, keepalive variants, watchdog) within the agreed 15–30 minute window.
...
- [x] Inspect the existing `reusable-16-agents.yml` job definitions and ensure timeout enforcement lives inside each job instead of the caller.
- [x] Audit `reusable-16-agents.yml` to confirm each long-running job has an explicit timeout; add or adjust values to stay within 15–30 minutes as appropriate.
