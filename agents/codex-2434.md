...
- Focus exclusively on the reusable workflow chain: `.github/workflows/reusable-16-agents.yml` and `agents-70-orchestrator.yml`; avoid unrelated workflow edits.
...
- Replace the broken JSON array concatenation in `reusable-16-agents.yml` using `format()` or by emitting well-formed JSON from the producing step—no new third-party actions.
...
1. `reusable-16-agents.yml` bootstraps issues labeled `agent:codex` without expression errors when run via `workflow_dispatch`.
...
- [x] Ensure `agents-70-orchestrator.yml` calls the reusable workflow with consistent inputs, defaults, and `if:` guards after the consumer wrapper retirement.
