...
- **Create a single entrypoint**: author `.github/workflows/selftest-runner.yml` that exposes the requested inputs (`mode`, `post_to`, `enable_history`) and runs the reusable CI matrix directly.
- **Retire wrapper scatter**: replace every existing self-test wrapper workflow with thin invocations of the new runner (or delete redundant files outright) while migrating the matrix logic into `selftest-runner.yml`.
...
- [x] Deleted the legacy `selftest-8X-*` wrappers so only the consolidated `selftest-runner.yml` remains.
