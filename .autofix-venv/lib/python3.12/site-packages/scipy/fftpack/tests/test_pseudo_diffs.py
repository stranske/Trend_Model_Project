...
class TestDiff:
        ...
        for n in [16, 17, 64, 127, 32]:
            ...
            for k in range(5):
                assert_array_almost_equal(
                    diff(sin(4 * x), k), direct_diff(sin(4 * x), k)
                )
                assert_array_almost_equal(
                    diff(cos(4 * x), k), direct_diff(cos(4 * x), k)
                )
    ...
    def test_period(self):
        for n in [17, 64]:
            x = arange(n) / float(n)
            assert_array_almost_equal(
                diff(sin(2 * pi * x), period=1), 2 * pi * cos(2 * pi * x)
            )
            assert_array_almost_equal(
    ...
    def test_definition(self):
        for h in [0.1, 0.5, 1, 5.5, 10]:
            for n in [16, 17, 64, 127]:
                ...
                assert_array_almost_equal(
                    tilbert(sin(2 * x), h), direct_tilbert(sin(2 * x), h)
                )
    ...
    def test_definition(self):
        for h in [0.1, 0.5, 1, 5.5, 10]:
            for n in [16, 17, 64, 127]:
                ...
                assert_array_almost_equal(y, y1)
                assert_array_almost_equal(
                    itilbert(sin(x), h), direct_itilbert(sin(x), h)
                )
                assert_array_almost_equal(
                    itilbert(sin(2 * x), h), direct_itilbert(sin(2 * x), h)
                )
