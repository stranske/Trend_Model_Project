...
class TestOddsRatio:
    ...
    def test_results_from_r(self, parameters, rresult):
        ...
        assert_allclose(
            (ci.low, ci.high), rresult.conditional_odds_ratio_ci, rtol=ci_rtol
        )
        ...
        if alternative == "two-sided":
            if ci.low > 0:
                sf = nchypergeom_fisher.sf(
                    table[0, 0] - 1, total, ngood, nsample, ci.low
                )
                assert_allclose(sf, alpha / 2, rtol=1e-11)
            if np.isfinite(ci.high):
                cdf = nchypergeom_fisher.cdf(
                    table[0, 0], total, ngood, nsample, ci.high
                )
