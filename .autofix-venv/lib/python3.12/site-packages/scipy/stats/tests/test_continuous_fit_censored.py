...
def test_beta():
    ...
    """
    data = CensoredData(
        interval=[[0.10, 0.20], [0.50, 0.55], [0.75, 0.90], [0.80, 0.95]]
    )
...
def test_gamma_right_censored():
    ...
    x = CensoredData.right_censored(
        [2.5, 2.9, 3.8, 9.1, 9.3, 12.0, 23.0, 25.0], [0] * 7 + [1]
    )
...
def test_nct():
    ...
    """
    data = CensoredData.right_censored(
        [1, 2, 3, 5, 8, 10, 25, 25], [0, 0, 0, 0, 0, 0, 1, 1]
    )
...
def test_ncx2():
    ...
    * 2 right-censored values [8, 8].
    ...
    """
    data = CensoredData(
        uncensored=[2.7, 0.2, 6.5, 0.4, 0.1], right=[8, 8], interval=[[0.6, 1.0]]
    )
    with np.errstate(over="ignore"):  # remove context when gh-14901 is closed
...
def test_weibull_censored1():
    ...
    times, cens = zip(
        *[(float(t[0]), len(t) == 2) for t in [w.split("*") for w in s.split(",")]]
    )
