...
def bytes_serializer(instance, widget):
    return {
        "data": (
            bytearray(memoryview(instance.data).tobytes()) if instance.data else None
        )
    }
...
def test_set_state_data_truncate(echo):
    ...
    assert (
        len(w.comm.messages) == 2 if echo else 1
    )  # ensure we didn't get more than expected
    ...
    assert msg == (
        (),
        dict(
            data=dict(method="update", state=dict(d={}), buffer_paths=[["d", "data"]])
        ),
    )
...
def test_no_echo(echo):
    ...
    widget._send.assert_has_calls(
        [
            mock.call(
                {"method": "update", "state": {"value": 43.0}, "buffer_paths": []},
                buffers=[],
            )
        ]
    )
