...
class Widget(LoggingHasTraits):
    ...
    def _handle_control_comm_msg(cls, msg):
        ...
        if method == "request_states":
            ...
            cls._control_comm.send(
                dict(
                    method="update_states", states=full_state, buffer_paths=buffer_paths
                ),
                buffers=buffers,
            )
            ...
            raise RuntimeError(
                'Unknown front-end to back-end widget control msg with method "%s"'
                % method
            )
    ...
    def set_state(self, sync_data):
        ...
        if JUPYTER_WIDGETS_ECHO:
            ...
            for attr, value in sync_data.items():
                if attr in self.keys and self.trait_metadata(
                    attr, "echo_update", default=True
                ):
                    echo_state[attr] = value
            if echo_state:
                echo_state, echo_buffer_paths, echo_buffers = _remove_buffers(
                    echo_state
                )
                msg = {
                    "method": "echo_update",
    ...
    def _send(self, msg, buffers=None):
        ...
        if self.comm is not None and (
            self.comm.kernel is not None if hasattr(self.comm, "kernel") else True
        ):
