...
- [x] Inventory the legacy summarizer workflows under `.github/workflows/` and
  ...
  table (legacy `maint-31` / `maint-32` versus consolidated Gate summary job).
- [x] Design the unified summary job so it
  triggers after the Gate jobs finish and
  scopes to pull-request runs. → Implemented in the Gate workflow with concurrency
...
- [x] Implement shared steps that download artifacts, render the consolidated
  ...
  `.github/workflows/pr-00-gate.yml` (summary job) plus
...
|----------|-------------|-------|
| `maint-31-pr-status-summary.yml` | Gate summary job (`pr-00-gate.yml`) | Formerly posted required vs optional job summary. Responsibilities folded into the consolidated script. |
| `maint-32-ci-matrix-summary.yml` | Gate summary job (`pr-00-gate.yml`) | Previously generated Markdown/JSON artifacts for coverage + failure snapshots. Superseded by the single PR comment. |
