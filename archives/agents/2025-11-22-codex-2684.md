...
- Update existing references only where required: the workflow system overview document and the header of `agents-63-issue-intake.yml` should link to the new policy. Avoid broader refactors or unrelated documentation edits.
...
- `.github/workflows/agents-63-issue-intake.yml` includes a header comment linking directly to the policy doc.
