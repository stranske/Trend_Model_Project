...
def identify_risk_free_fund(df):
    ...
    stdevs = {col: df[col].dropna().std() if not df[col].dropna().empty else np.inf
              for col in cols}
...
# --- 6. Select Funds ---
def select_funds(df, rf_col, fund_columns, in_sdate, in_edate, out_sdate, out_edate,
                 selection_mode='all', random_n=8):
...
def get_custom_weights(selected_funds):
    weight_widgets = {f: widgets.BoundedIntText(value=0, min=0, max=100,
                      description=f, layout=widgets.Layout(width='250px')) 
